...
export const addAddress = (savedAddressArray:savedAddress[], addressToAdd:savedAddress):savedAddress[] => {
    const tempAddresses = JSON.parse(JSON.stringify(savedAddressArray)) as savedAddress[];
    ...
    if (tempAddresses.length < maxSavedAddresses)
    {
        ...
        tempAddresses.map((address, index) => {
            if ((FoundIndex === -1) && JSON.stringify(address) === JSON.stringify(addressToAdd))
            {
                FoundIndex = index
            }
        })
