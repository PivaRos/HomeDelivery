...
const OptionProductTab = (props: Props) => {
  ...
  useEffect(() => {
    ...
    JSON.stringify(props.isChecked),
    JSON.stringify(props.optionProductUnits[props.index]),
  ]);
      ...
      if (
        (props.option.additionalAllowed &&
          props.option.maxPicks + props.option.additionalMax >
            getTotalUnits(props.optionProductUnits)) ||
        (!props.option.additionalAllowed &&
          props.option.maxPicks > getTotalUnits(props.optionProductUnits))
      ) {
  ...
  const setChecked = (isChecked: boolean) => {
    let ToptionProductCheckedState = JSON.parse(
      JSON.stringify(props.optionProductCheckedState)
    );
