...
export const ProductOptionsList = forwardRef(
  (props: Props, ref: React.ForwardedRef<unknown>) => {
    const [optionProductCheckedState, setOptionProductCheckedState] = useState<
      boolean[]
    >(
      props.option.optionProducts.map(() => {
        return false;
      })
    );
    const [optionProductUnits, setOptionProductUnits] = useState<number[]>(
      props.option.optionProducts.map(() => {
        return 0;
      })
    );
    ...
    useEffect(() => {
      if (
        props.option.selectedOptionProducts &&
        props.option.selectedOptionProducts.length > 0
      ) {
        //if product is not default *
        // need to update checkedState
        setOptionProductCheckedState(
          props.option.selectedOptionProducts.map((v) => {
            return v.selected;
          })
        );
        // need to update units
        setOptionProductUnits(
          props.option.selectedOptionProducts.map((v) => {
            return v.units;
          })
        );
    ...
    const getLimitString = (): string => {
      if (!props.option.mustPicks) {
        if (
          props.option.additionalAllowed &&
          props.option.maxPicks + props.option.additionalMax > 1
        ) {
          return `ניתן לבחור עד ${
            props.option.maxPicks + props.option.additionalMax
          } פריטים ${props.option.maxPicks} בחינם `;
        }
    ...
    useEffect(() => {
      ...
      JSON.stringify(optionProductCheckedState),
      JSON.stringify(optionProductUnits),
    ]);
    ...
    return (
      ...
      >
        <Text style={{ fontWeight: "bold", padding: 5, paddingLeft: 15 }}>
          {props.option.name}
        </Text>
        <ShakeText
          duration={100}
        ...
        >
          {getTotalUnits(optionProductUnits) > props.option.maxPicks &&
            PriceString(
              props.option.additionalPricePerUnit.price *
                (getTotalUnits(optionProductUnits) - props.option.maxPicks),
              props.option.additionalPricePerUnit.currency
            ) + " + "}{" "}
          {getLimitString()}
        </ShakeText>
        {props.option.optionProducts.map((OptionProduct, index) => {
          return (
            <OptionProductTab
              Shake={Shake}
              checkIfNeedUpdate={props.checkIfNeedUpdate}
              option={props.option}
              optionProductCheckedState={optionProductCheckedState}
              optionProductUnits={optionProductUnits}
              setOptionProductUnits={setOptionProductUnits}
              isChecked={optionProductCheckedState[index]}
              setOptionProductCheckedState={setOptionProductCheckedState}
              key={index}
              index={index}
              optionProduct={GetOptionProduct(props.store, OptionProduct)}
            />
          );
        })}
      </View>
