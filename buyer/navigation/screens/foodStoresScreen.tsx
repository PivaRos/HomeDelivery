...
const FoodStores = (props: Props) => {
  ...
  const foodStores = useSelector(
    (state: any) => state.foodStores
  ) as availableStores;
  ...
  const load = async () => {
    try {
      if (foodStores.Open.length === 0 && foodStores.Closed.length === 0) {
  ...
  return (
    <View style={style.view}>
      {foodStores &&
        (JSON.stringify(foodStores.Open) !== JSON.stringify([]) ||
          JSON.stringify(foodStores.Closed) !== JSON.stringify([])) && (
      ...
      (JSON.stringify(foodStores.Open) !== JSON.stringify([]) ||
        ...
        <ScrollView>
          <StoresGrid
            ...
            displayStores={foodStores?.Open}
          />
          <StoresGrid
            title="Closed Stores"
            displayStores={foodStores?.Closed}
          />
        </ScrollView>
      ) : (
        !loading && (
